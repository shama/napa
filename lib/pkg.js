...
function NapaPkg(url, name, opts) {
  ...
  Object.defineProperty(self, 'cached', {
    get: function() {
        return (self.execOpts.cache) ? fs.existsSync(self.cacheTo) : false
    }
  })
}
...
NapaPkg.prototype.install = function(done) {
  ...
  if (self.installed) {
    if (self.execOpts.force) {
        self.log.info('force', self.name, '(already installed)')
    } else {
        self.log.info('ignore', self.name, '(already installed)')
        return done()
    }
  }
  ...
  function cb(err) {
    ...
    self.writePackageJson(function(err) {
      ...
      self.log.info('done', self.name)
      if (self.execOpts.cache) {
        self.cache(done)
      } else {
        done()
      }
  ...
  function cacheInstall() {
    ...
    if (typeof self._mock === 'function') {
      ...
      self.log.info(self.method, '%s into %s', self.url, self.name)
